class GeosController < ApplicationController
  ...
  def show
    ...
    @facilities = @geo.facilities.order('site_name').page(params['page']).per(100)
  end
  ...
  def create
    @geo = Geo.new(geo_params)
      ...
      if @geo.save
        redirect_to @geo, notice: 'Track was successfully created.'
      else
        render :new
      end
  ...
  private
    ...
    def geo_params
      params.require(:geo).permit(:title, :tracker)
    end
end
